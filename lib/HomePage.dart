...
class _HomePageState extends State<HomePage> {
                ...
                return StreamBuilder<Position>(
                    ...
                    builder: (context, snapshot) {
                      if (!snapshot.hasData) {
                        ...
                        return Loading(completed: Text('Carregando Mapa'));
                      }
              ...
              return StreamBuilder<List<Requisicao>>(
                  ...
                  builder: (context,
                    ...
                    return StreamBuilder<List<Motorista>>(
                        ...
                        builder: (context, AsyncSnapshot<List<Motorista>> snapMoto)
                    {
                 if(snapMoto.data == null){
                   return Container();
                 }
                      if (requisicao.data == null) {
                        ...
                        for (var req in requisicao.data) {
                          if (req.aceito == null) {
                            if (req.user == Helper.localUser.id) {
  ...
  requisicao( latInicial, lngIncial, String destinoFinal) async {
    Geolocator()
        .getCurrentPosition(desiredAccuracy: LocationAccuracy.high)
        .then((v) async {
      ...
      List<Placemark> inicial =
      await Geolocator().placemarkFromCoordinates(latInicial, lngIncial);




      _initialPosition = LatLng(latInicial,lngIncial);

      LatLng destination = LatLng(latitude, longitude);
      dToast('Efetuando calculo de rota');

      rc.CalcularRota(_initialPosition, destination);
      ...
      endereco_origem = Endereco(
          numero: inicial[0].name,
          endereco: inicial[0].thoroughfare,
          bairro: inicial[0].subLocality,
          cidade: inicial[0].locality,
          estado: inicial[0].administrativeArea,
          lat: latInicial,
          lng: lngIncial,
          cep: inicial[0].postalCode);
...
print('aqui a soma ${soma}');
    ...
    print('aqui a requisicao ${requisicao}');
    await  criaRc.CriarRequisicao(requisicao);
