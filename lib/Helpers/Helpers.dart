...
import 'package:ufly/Helpers/NotificacoesHelper.dart';
...
const Color vermelho = Color.fromARGB(255, 128, 57, 226);
  ...
  return GestureDetector(
    ...
    child: Row(
      children: <Widget>[
        ...
        sb,
        hText(text, context, size: 30, color: vermelho),
      ],
    ),
  );
}
...
class Helpers {
  static User user;
  static Endereco selectedAddress;
  static FirebaseApp app;
  static NotificacoesHelper nh = new NotificacoesHelper();
//2019-02-14 14:42:38
  static DateFormat df = DateFormat('yyyy-MM-dd kk:mm:ss');
  static String token;

  static FirebaseMessaging fbmsg;

  Map<String, dynamic> getmapListObject(Map<int, dynamic> d) {
    Map<String, dynamic> s = new Map();
    void iterateMapEntry(key, value) {
      if (value != null) {
        s[key.toString()] = value.toJson();
      } else {
        s[key.toString()] = '';
      }
    }
  ...
  TextStyle getStyle(
      {Color c, int size, String font, bool isItalic, bool isBold}) {
    return GoogleFonts.openSans(
        color: c == null ? Colors.red : c,
  ...
  Map<String, dynamic> getmapList(Map<dynamic, dynamic> d) {
    Map<String, dynamic> s = new Map();
    void iterateMapEntry(key, value) {
  ...
  String Capitalize(String descricao) {
    try {
      var s = descricao.toLowerCase().split(' ');
      String f = '';
      for (String i in s) {
        //print('AQUI VALUE' + i);
        var j = i[0].toUpperCase();
        i = i.replaceFirst(i[0], j);
        f = f + i + ' ';
      }
      return f;
    } catch (err) {
  ...
  reduceStrSize(String capitalize) {
    print('Entrou AQUI');
    String s = capitalize.replaceRange(15, capitalize.length, '');
    return s + '...';
  }
}
